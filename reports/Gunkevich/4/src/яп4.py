...
def task_1_2():
    ...
    for i in list:
        if (list.index(i) + 1) % 2 == 0 : print("Чётные элементы в списке:", i)
...
def task_2_2():
    list = ["cucumber", "tomato", "potato", "monster"]
    for i in list:
        if i[-1] == "r": print(i)
def task_2_3():
    while True:
...
def task_3_7(list):
    ...
    #Задание 4
def task_4_1():
    string = "Счастье, это когда хочешь делать то, что тебя заставляют."
    l = string.split(' ')
    string = ""
    for i in l :
...
def task_4_4():
    s = "Очень содержательное и длинное предложение"
    syms = len(s)
    syms2 = len(s) - s.count(' ')
    words = s.count(' ') + 1
...
def task_3(list):
            while True:
                cmd = input("Введите номер пункта задания(условия для третьего задания отсутствуют, поэтому его нет :3 ):")
                if int(cmd) == 1:
                    task_3_1(list)
                elif int(cmd) == 2:
                    task_3_2(list)
